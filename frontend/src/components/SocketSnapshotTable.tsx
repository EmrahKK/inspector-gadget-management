...
export const SocketSnapshotTable: React.FC<Props> = ({ outputs }) => {
  ...
  const getStatusColor = (status: string) => {
    ...
    if (s === 'LISTEN') return 'text-blue-400';
    if (s === 'ESTABLISHED' || s === 'CONNECTED') return 'text-green-400';
    ...
    <div className="h-full flex flex-col bg-slate-900">
      ...
      <div className="border-b border-slate-700 p-4 bg-slate-800/50">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h3 className="text-lg font-semibold text-white">Socket Snapshot</h3>
          ...
          <tbody>
            {filteredAndSortedSockets.map((socket, index) => (
              ...
              >
                ...
                <td className="p-3 text-slate-300 font-mono text-xs">{socket.pod}</td>
                <td className="p-3">
                ...
                <td className="p-3">
                  <span className={`text-xs px-2 py-1 rounded font-mono ${
                    socket.protocol.toUpperCase() === 'TCP' ? 'bg-purple-500/20 text-purple-400' :
                    socket.protocol.toUpperCase() === 'UDP' ? 'bg-orange-500/20 text-orange-400' :
                ...
                <td className="p-3 text-amber-300 font-mono text-xs">
                  {socket.remotePort || '-'}
                </td>
                <td className="p-3">
                ...
                <td className="p-3 text-slate-400 font-mono text-xs">{socket.uid}</td>
              </tr>
            ))}
          </tbody>
        </table>
